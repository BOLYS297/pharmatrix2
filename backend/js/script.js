...
$(document).ready(function () {
  ...
  function loadMedicaments() {
    ...
    select
      .empty()
      .append('<option value="">Sélectionnez un médicament</option>');

    medicaments.forEach((med) => {
      select.append(
        `<option value="${med.id}" data-prix="${med.prix}">${med.nom} - ${med.prix} FCFA</option>`
      );
    });
  }
    ...
    if (step === 1) {
      ...
      if (isValid) {
        ...
        commandeData.prix_total =
          commandeData.medicament.prix * commandeData.quantite;
      }
    } else if (step === 3) {
      ...
      if (isValid) {
        $("#confirmationClient").text(
          `${commandeData.client.nom} ${commandeData.client.prenom}`
        );
        $("#confirmationTelephone").text(commandeData.client.telephone);
        $("#confirmationEmail").text(
          commandeData.client.email || "Non renseigné"
        );
        $("#confirmationMedicament").text(
          `${commandeData.medicament.nom} (${commandeData.medicament.prix} FCFA)`
        );
        $("#confirmationQuantite").text(commandeData.quantite);
        $("#confirmationPrixUnitaire").text(
          `${commandeData.medicament.prix.toLocaleString("fr-FR")} FCFA`
        );
        $("#confirmationTotal").text(
          `${commandeData.prix_total.toLocaleString("fr-FR")} FCFA`
        );
      }
    }
